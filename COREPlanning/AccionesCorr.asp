...
if ok <> "Y" then
  ...
  sHTML = ofv.FormHeader("ACCIONES CIRCUITO CORRECTIVO")
  ...
  if rs.eof then
    ...
    Do While Not rs.EOF and posi < CantRegMostrarx
      ...
      if bSuprimir then
        ...
        sHref = sHref & "&Tabla=Tipoacccorr&volver=" & vuelta & "&Eliminar=NSNC"
      ...
      'Descripcion
      sInput = ofv.GenerarInput(rs(1).name,rs(1),"text",sForm,"145","dt MAXLENGTH=100","")
      ...
      if bModificar then
        ...
        sAccion = sAccion & "&tabla=Tipoacccorr&volver=" & vuelta
  ...
  Agregar = ""
  if bAgregar then
    sAccion = "AgregarAccionesCorr.asp?Regisvuelta=" & CantRegMoverx
    sAccion = sAccion & "&Opcion=si&tabla=Tipoacccorr"
    Agregar = ofv.BotonAgregar(sAccion)
  end if

  str1 = "Select descri From Tipoacccorr Order By descri "    ' orden pagina
  str2 = "Select descri From Tipoacccorr Order By descri "  ' orden alfabetico
