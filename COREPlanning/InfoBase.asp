...
if ok <> "Y" then
  ...
  sHTML = ofv.FormHeader("INFORMACION DE BASE")
  ...
  if rs.eof then
    ...
    Do While Not rs.EOF and posi < CantRegMostrarx
      ...
      if bSuprimir then
        ...
        sHref = sHref & "&Tabla=Infrespcorr&volver=" & vuelta & "&Eliminar=NSNC"
      ...
      if bModificar then
        ...
        sAccion = sAccion & "&tabla=Infrespcorr&volver=" & vuelta
  ...
  Agregar = ""
  if bAgregar then
    sAccion = "AgregarInfoBase.asp?Regisvuelta=" & CantRegMoverx
    sAccion = sAccion & "&Opcion=si&tabla=Infrespcorr"
    Agregar = ofv.BotonAgregar(sAccion)
  end if

  str1 = "Select descri From Infrespcorr Order by Descri "    ' orden pagina
  str2 = "Select descri From Infrespcorr Order by Descri "  ' orden alfabetico
