...
if ok <> "Y" then
  ...
  sHTML = ofv.FormHeader("CIRCUITOS")


  'Encabezados
  Formulario = ofv.Encabezados("Tema","Orden")
...
' Valida si no hay registros
   if rs.eof then
      sHTML = sHTML & "<p align=center ><font face=tahoma size=1 color=#990000><b>NO HAY ELEMENTOS ASOCIADOS, INGRESE TEMAS PRIMERO </b><br><br></FONT></P>"
   Else 
    ...
    Do While Not rs.EOF and posi < CantRegMostrarx
      ...
      'Orden
      sInput = ofv.GenerarInput(rs(2).name,rs(2),"text",sForm,"10","dt MAXLENGTH=5","SIR")
      Celda = Celda & ofv.GenCelda("","","","","","","",sInput,"si")
...
' Bifurcacion a Subtemas

      sForm1 = "Boton" & posi
      sInput = ofv.GenerarInput("","Subtemas","submit",sForm1,"10","bt","")
      Celda =  ofv.GenCelda("","","","","","","",sInput,"si")
      Celda = Celda & ofv.genfrow()
      sAccion = "Subtemas.asp?cantregmostrar=0" & "&Tema=" & rs(0) & "&CantRegMover=" & CantRegMoverx

      sAccion = sAccion  & "&Temadesc=" & ofv.ConvertirCar(rs(1)," ","*") 
  ...
  Agregar = ""
  if bAgregar then
    sAccion = "AgregarCircuito.asp?Regisvuelta=" & CantRegMoverx & "&Opcion=si&tabla=TEMA"
