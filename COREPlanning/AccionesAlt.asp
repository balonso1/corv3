...
if ok <> "Y" then
  ...
  sHTML = ofv.FormHeader("ACCIONES CIRCUITO ALTERNATIVO")
  ...
  if rs.eof then
    ...
    Do While Not rs.EOF and posi < CantRegMostrarx
      ...
      if bSuprimir then
        ...
        sHref = sHref & "&Tabla=Tipoaccalt&volver=" & vuelta & "&Eliminar=NSNC"
      ...
      if bModificar then
        ...
        sAccion = sAccion & "&tabla=Tipoaccalt&volver=" & vuelta
  ...
  Agregar = ""
  if bAgregar then
    sAccion = "AgregarAccionesAlt.asp?Regisvuelta=" & CantRegMoverx
    sAccion = sAccion & "&Opcion=si&tabla=Tipoaccalt"
    Agregar = ofv.BotonAgregar(sAccion)
  end if

  str1 = "Select descri From Tipoaccalt Order By descri "    ' orden pagina
  str2 = "Select descri From Tipoaccalt Order By descri "  ' orden alfabetico
