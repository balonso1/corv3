...
sHTML = FormHeader("ELIMINAR PROVEEDOR")
...
' reviso que no figure en la tabla CIRCORR
	strsql="select * from CIRCORR Where Codprovprinc =" & tipoID & ID & tipoID & " or Codprovalt =" & tipoID & ID & tipoID
	set rs = crearconsultaEx(StrSql,cn,1,volver)
'response.write strsql & chr(13)

	if not rs.eof then  
	   sMensaje = "No puede eliminar al proveedor * " & NOM & " * porque esta asociado "
	   sMensaje = sMensaje & " a uno o mas circuitos correctivos. Debe eliminarlos previamente."
	   Boton = 1
	else
	   sMensaje = "Usted esta a punto de eliminar al proveedor * " & NOM & " * definitivamente."  
	   Boton = 2
	end if
	call cerrarconsulta(rs)
