...
sHTML = FormHeader("ELIMINAR CASO DE CONTINGENCIA")
...
' reviso que no figure en la tabla CONTINGENCIA
	strsql="select * from CONTINGENCIA Where Tipocontin =" & tipoID & ID & tipoID
	set rs = crearconsultaEx(StrSql,cn,1,volver)
'response.write strsql & chr(13)

	if not rs.eof then  
	   sMensaje = "No puede eliminar la contingencia * " & NOM & " * porque esta asociada "
	   sMensaje = sMensaje & " a uno o mas circuitos de contingencia. Debe eliminarlos previamente."
	   Boton = 1
	else
' reviso que no figure en la tabla ACCIONALT
	   call cerrarconsulta(rs)
	   strsql="select * from ACCIONALT Where disparador =" & tipoID & ID & tipoID
	   set rs = crearconsultaEx(StrSql,cn,1,volver)
	      if not rs.eof then  
	   	sMensaje = "No puede eliminar la asociacion * " & NOM & " * porque esta relacionada "
	   	sMensaje = sMensaje & " a uno o mas circuitos alternativos. Debe eliminarlos previamente."
	   	Boton = 1
	      else
	   	sMensaje = "Usted esta a punto de eliminar la contingencia * " & NOM & " * definitivamente."  
	   	Boton = 2
	      end if
	end if
	call cerrarconsulta(rs)
