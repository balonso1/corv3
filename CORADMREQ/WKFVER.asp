...
if ok <> "Y" then
  ...
  sHTML = sHTML  & " document.title='COR OnTime - Consulta de WorkFlow'; "
 ...
 ' sHTML = sHTML  & " alert(Datos); "
  ...
  sHTML = sHTML  & " myWin= open(Datos," & chr(34) & "ConsP" & chr(34) & "," & chr(34) & "RESIZABLE=YES,menubar=no,toolbar=no,height=570,width=790" & chr(34) & " ); "
  sHTML = sHTML  & " } "
  sHTML = sHTML  & "</script>"
 ...
 Whtml = Whtml & "<table border=0 cellspacing=0 cellpadding=0 bgcolor=#eeeeee width=100% height=100% style='border-style:ridge;border-width:3;' >"
 Whtml = Whtml & "<tr>"
 Whtml = Whtml & "<td align=center style='border-style:ridge,border-width:2; filter:shadow(color=#777777,direction=135);' >"
 Whtml = Whtml & "<IMG border=0 src='../coress20/imagenes/corlogo.gif' style='filter: chroma(color=#ffffff);' >"
 Whtml = Whtml & "<FONT color=#ffffff face=verdana size=2>"
 Whtml = Whtml & "<B>&nbsp;Cor&nbsp;Consulting</B>"
 Whtml = Whtml & "</FONT>"
 Whtml = Whtml & "</td></tr></table>"


    sInput = "<B><h3>&nbsp;COR&nbsp;<i><font color=#f7aa22>On</font></i>Time&nbsp;-&nbsp;Consulta&nbsp;de&nbsp;WorkFlow</h3></B>"
    celdas = ofv.GenCelda("","vt  ","","80%","","","",sInput,"si")

    sInput = Whtml
    celdas = celdas & ofv.GenCelda("","","","200","","","",sInput,"si")

    filas = ofv.GenRow("","","","","","","",celdas,"si")

  sHTML = sHTML  & ofv.GenTabla("","AAaa  ","","98%","","0","","-2","-1","","",filas,"si")
  ...
  if not rs.eof then
   ...
   IF   CKTOPC = "SI" THEN
    ...
    sInput = CKTTIPO
    celdas = ofv.GenCelda("","vt","","40%","","","",sInput,"si")

    sInput = "<B>" & CKTDESCRI & " - " & CKTcodiden & "</B>"
    celdas = celdas & ofv.GenCelda("","vt","","60%","","","",sInput,"si")
  ...
  sHTML = sHTML  & ofv.GenTabla("","AA STYLE='border-style:ridge;border-width:2;border-color:#6699cc;filter:shadow(color=#dddddd,direction=125);' ","","98%","","0","","0","0","","",filas,"si") & "<BR>"
...
'''        DFD    = TPASORD(IPO3,1) - (4 + TPASOK(PASRCH,13) ) + BFILAD
 ...
 NEXT  
      ...
      STCB  = "COLOR:" & colr
      STB  = "BORDER-STYLE:SOLID;BORDER-WIDTH:1;BORDER-COLOR:" & colr
      STBL = "BORDER-LEFT-STYLE:SOLID;BORDER-LEFT-WIDTH:1;BORDER-LEFT-COLOR:" & colr
      STBR = "BORDER-RIGHT-STYLE:SOLID;BORDER-RIGHT-WIDTH:1;BORDER-RIGHT-COLOR:" & colr
      STBT = "BORDER-TOP-STYLE:SOLID;BORDER-TOP-WIDTH:1;BORDER-TOP-COLOR:" & colr
      STBB = "BORDER-BOTTOM-STYLE:SOLID;BORDER-BOTTOM-WIDTH:1;BORDER-BOTTOM-COLOR:" & colr
...
'      STRBU = "BORDER-BOTTOM-STYLE:DOTTED;BORDER-BOTTOM-WIDTH:2;BORDER-BOTTOM-COLOR:" & colr
      ...
      '''    celdas = celdas & ofv.GenCelda("","TT COLSPAN=8","CENTER","1%","","","",sInput,"si")
          ...
          celdas = celdas & ofv.GenCelda("","vT COLSPAN=8 style='border-style:ridge;border-width:2;border-color:#6699cc;' valign=middle ","CENTER","1%","20","","",sInput,"si")
      NEXT
...
'                END IF
                ...
                IF (IGNIMF <> IMF) AND (IGNIMF <> (IMF - 1)) THEN
                   ...
                   IF TPASOK(IPO,14) = "S" THEN
                      ...
                      if CKTOPC = "SI" then
                         ...
                         XFUNCION = "  onclick=" & chr(34) & "AbrirCons('SEGCKTESTPSC.asp?PSC=" & xcktpsc & "'); return false;" & chr(34)
                      else
                         xtipo   = "WKF"
                         xmodel  = TPASOK(IPO,2)
                         xcktpsc = "NO"
                      ...
                      celdas = celdas & ofv.GenCelda("","vt style='cursor:hand;border-style:ridge;border-width:2;border-color:#f7aa22;'  COLSPAN=4 ROWSPAN=2 " & XFUNCION & " ","CENTER","1%","30","","",XPASO,"si")
                   ELSE
   ...
   sHTML = sHTML  & ofv.GenTabla("","AA STYLE='border-style:ridge;border-width:2;border-color:#6699cc;filter:shadow(color=#dddddd,direction=125);' ","","90%","","0","","-1","-2","","",filas,"si") & "<BR>"
        ...
        ELSE
           ...
           IF XT = "O" THEN
              ...
              FOR IM=DF+1 TO DFD-1
                  ...
                  IF ((IM - (DF+1))  / (5 * IMR)) = 1 THEN
                     IMR = IMR + 1
                     IF IDX < 7 THEN XMATRIX(IM,DC,0) = "C2"
                  END IF
              NEXT
           ELSE
          ''' FOR IM=DF TO DFD-1
