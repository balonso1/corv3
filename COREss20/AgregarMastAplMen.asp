...
if ok <> "Y" then
  ...
  sid2 = "CLI-" & stipo & ssub & cstr(sid)
  sid = "CLI" & stipo & ssub & cstr(sid)


  strsql = "Select max(morden) From " & tabla & " where mdepen = '" & sdepen
  strsql = strsql & "' and mcodapli = '" & sapli & "' " 
  set rs = ofv.crearconsultaEx(StrSql,cn,1,volver)
  if not rs.eof then
     if not isnull(rs(0)) then sor = clng(rs(0))
     sor = sor + 1
  end if
  call ofv.cerrarconsulta(rs)
...
sHTML = sHTML & "</script>" &  chr(13) 
    ...
    f1a = "asp/AgregarMastAplMen2.asp?Tabla=" & Tabla
    f1a = f1a & "&volver=" & Volver & "&codapli=" & sapli & "&alias=" & sid 
    f1a = f1a & "&ATRM=" & sid2 & "&depen=" & sdepen & "&stipo=" & stipo  & "&ssub=" & ssub 
  ...
  'Encabezado Accion
    ...
    sInput = ofv.GenerarInput("Menu","","agregar","","100","dt","")
    celdas = celdas & ofv.GenCelda("","cc","","","","","",sInput,"si")

  sHTML = sHTML & ofv.GenRow("","","","","","","",celdas,"si")

    sInput1 = ofv.GenerarInput("mcodapli",sapli,"hidden","","1","dt","")
    sInput2 = ofv.GenerarInput("Asp","InitClApl.asp","hidden","","1","dt","")
    sInput3 = ofv.GenerarInput("Menuatr",sid,"hidden","","1","dt","")
    sInput4 = ofv.GenerarInput("morden",sor,"hidden","","1","dt","")
    sInput5 = ofv.GenerarInput("mdepen",sdepen,"hidden","","1","dt","")
    sInput6 = ofv.GenerarInput("mun","1","hidden","","1","dt","")
    celdas =  ofv.GenCelda("","aaa colspan=2","","","","","",sInput1 & sInput2 & sInput3 & sInput4 & sInput5 & sInput6,"si")
