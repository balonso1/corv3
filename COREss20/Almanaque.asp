...
if ok <> "Y" then
  ...
  xvuelta=request.querystring("svuelve")
...
'  cnom = request.querystring("Cnom")
 ' cx = request.querystring("Cx")
  ...
  fechaxA=request.form("FF")&""
  
 
  
  if fechaxA="" then
   fechaxt= request.querystring("xcampo")&""
   fechaxA= request.querystring(fechaxt)
    if fechaxA="" then 
     'fechax=clng(date) - clng(cdate("01-01-1900"))
     fechaxA=clng(date) 
    end if
  end if
  
  
  'response.write fechax &"<br>"
  rs(1)=clng(fechaxA) ' clng(date) - clng(cdate("01-01-1900"))
  

  sHTML = ofv.FormHeader("Almanaque") & "<CENTER><BR>"

  'Encabezados
  Formulario = ""

'response.write fechax
  ...
  'Strsql = "Select * From feriados "'where id =-1 "
 ...
 ' if not rs.eof then 
  posi = 0
   ...
   ' sanioC = datepart("yyyy",date())
   ' smes = datepart("m",date())
   
   sanio = datepart("yyyy",fechaxA)
   sanioC = datepart("yyyy",fechaxA)
   smes = datepart("m",fechaxA)
    ...
    IF CA <> "" THEN 
       smesc = request.form("smesc")
        sanioC = request.form("sanioc")
         IF (LEN(SANIOC&"")=0) OR CSTR(SANIOC) = "0" THEN SANIOC = SANIO
  ...
  '     sanioc = sanio
   ...
   ' end if
    ...
    'codigo
     ...
     sInput = ofv.GenerarCombo(sForm & "A","smesc",smesc,Strsql,cn,1,2,"sb")
     Celda = Celda & ofv.gencelda("",sclase,"","40%","","","",sInput,"si")

     Celda = Celda & ofv.GenCelda("","TT","","10%","","","","año:","si")
     
     'sinput = sanioc 
     sInput = "<font class=at><select name=SANIOC onchange=submit()>"
     
             FOR XYY=-10 TO 7
     
     
                 sInput = sInput & "<option value=" & (CINT(SANIO) + XYY)
                 if CINT(CINT(SANIO) + XYY) = CINT(SANIOC) then sInput = sInput & " selected"
                 sInput = sInput & " >" & (CINT(SANIO) + XYY)
    ...
    if bModificar then
     ...
     for each xx in request.querystring
      if xx<> "ca" THEN
         str=str & xx &"=" & request.querystring(xx) & "&"
      END IF   
     next 
    
      sAccion = "almanaque.asp?ca="& smesc &"&" &str
      'response.write saccion
...
'           sAccion = sAccion & "&tabla=feriados&volver=" & vuelta
            ...
            sAccion = "almanaque.asp?"& str 'request.querystring

           t1 = ofv.GenerarInput("bvo1",sInput1,"submit","","10",sclase,"")
           t1 = t1 & ofv.GenerarInput("FF",(afechs(ii,1) - srest),"hidden","","2","dt","")
 ...
 ' response.write vuelta
  xcampo=request.querystring("xcampo")
  str=""
  for each xx in request.querystring
  if xcampo=xx then
   str=str & xx &"=" & fechaxA & "&"
  ...
  svuelve ="../CORADMREQ/"& session("svuelveAlm") & "?" & str
