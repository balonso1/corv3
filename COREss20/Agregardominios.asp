...
if ok <> "Y" then
  ...
  sHTML = ofv.FormHeader("AGREGAR Dominio")
  ...
  'Descripcion
  sInput = ofv.GenerarInput(rs(1).name,"","agregar","","120","dt","")
 ' sInput1 = ofv.GenerarInput(rs(0).name,"0","hidden","","120","dt","")
 ...
 ' Celda = Celda & ofv.gencelda("","","","","","","","","si")
  ...
  sFuncion = sFuncion & "this." & rs(1).name & ".value.length==0) "
  sFuncion = sFuncion & "{alert('Observacion no debe ser nulo');return false;} "
  sFuncion = sFuncion & chr(34)
  sAccion = "asp/Agregar.asp?Tabla=" & Tabla & "&volver=../dominios.asp?CantRegMover="
  sAccion = sAccion & Regisvuelta
  Formulario = Formulario & ofv.GenForm("Agregarb","",sAccion,"","post",sFuncion,Fila,"no")
  ...
  'Aceptar y Cancelar
  sAccion = "dominios.asp?CantRegMover=" & Regisvuelta
